...
function Sports() {
  ...
  async function FetchResources() {
    try {
      ...
      setResources(
        response.filter((index: Resource) => index.category === "sports")
      );
    } catch (error) {
      console.log(error);
  ...
  const handleImageClick = (event: React.MouseEvent<HTMLImageElement>) => {
    ...
    const resourceCollectionElement = document.querySelector(
    ...
    if (foundResource) {
      ...
      resourceCollectionElement.href = `/photos/categories/${foundResource.category}`;
    }
  };
  ...
  return resources.length > 0 ? (
    <>
      ...
      <section className="sports">
        <h1>Beautiful photos from sports</h1>
        <p>
